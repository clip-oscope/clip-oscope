...
all_sentences = [comb for comb in combinations(all_shapes, n)]
...
shape_features = shape_features.to('cpu').type(torch.float32)
...
for i in tqdm(range(0, len(all_sentences), batch_size), desc="Processing sentences"):
    ...
    for n_comb in batch_sentences:
        zero_torch = torch.zeros(embedding_size)
        for comb in n_comb:
            zero_torch += shape_features[all_shapes.index(comb), :]
        batch_embeddings.append(zero_torch)
    batch_embeddings = torch.stack(batch_embeddings, dim=0)

    cos_similarities = torch.mm(batch_embeddings, shape_features.t()).cpu().numpy()
    ...
    for j, sentence in enumerate(batch_sentences):
        ...
        if all(word in sentence for word in topn_words):
            ...
            for k in range(n):
                if most_similar_word == sentence[k]:
