# download the captions file: !gdown 12ni2gB-qTQIUp2H8Fgc5gzINW0o2Zw6z
import torch
...
for index, row in tqdm(df.iterrows()):
    ...
    for obj in all_shapes:
        if obj in caption.split():
            temp_obj_list.append(obj)
            temp_obj_index.append(caption.index(obj))
    final_obj_list = sort_list(temp_obj_list, temp_obj_index)
    if len(final_obj_list) == n:
        all_sentences.append(caption)
        object_list.append(final_obj_list)
...
for i in tqdm(range(0, len(all_sentences), batch_size), desc="Processing sentences"):
    ...
    for j, sentence in enumerate(batch_sentences):
        ...
        words_in_sentence = object_list[i+j]
        ...
        if all(word in words_in_sentence for word in topn_words):
            topn_correct += 1
