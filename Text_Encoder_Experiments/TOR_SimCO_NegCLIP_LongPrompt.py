...
prompt2 = "This vibrant display features a stunning OBJ1 with its radiant glow, a mesmerizing OBJ2 with bold contours."
prompt3 = "This vibrant display features a stunning OBJ1 with its radiant glow, a mesmerizing OBJ2 with bold contours, an enchanting OBJ3 that fits perfectly with its graceful form."
prompt4 = "This vibrant display features a stunning OBJ1 with its radiant glow, a mesmerizing OBJ2 with bold contours, an enchanting OBJ3 that fits perfectly with its graceful form, a dazzling OBJ4 with brilliant tones and intricate patterns."
prompt5 = "This vibrant display features a stunning OBJ1 with its radiant glow, a mesmerizing OBJ2 with bold contours, an enchanting OBJ3 that fits perfectly with its graceful form, a dazzling OBJ4 with brilliant tones and intricate patterns, and an alluring OBJ5 that completes the ensemble with its seamless fusion and distinct shape."


all_shapes = ['cube', 'sphere', 'cylinder', 'mug', 'pentagon', 'heart', 'cone', 'pyramid', 'diamond', 'moon', 'cross', 'snowflake', 'leaf', 'arrow', 'star', 'torus', 'pot', 'cap']

all_combinations = list(combinations(all_shapes, n))
all_sentences = list()
for comb in all_combinations:
    ...
    for i in range(n):
        prompt = prompt.replace(f"OBJ{i+1}", comb[i])
...
for i in tqdm(range(0, len(all_sentences), batch_size), desc="Processing sentences"):
    ...
    for j, sentence in enumerate(batch_sentences):
        ...
        words_in_sentence = list(all_combinations[i+j])
